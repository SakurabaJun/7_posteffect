    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            fixed4 frag (v2f_customrendertexture i) : SV_Target
            {
                ...
                col += w1 * tex2D(_MainTex, i.globalTexcoord + float2(1.5,0) * scale);
                col += w1 * tex2D(_MainTex, i.globalTexcoord - float2(1.5,0) * scale);
                weight +=2.0*w1;
                
                float w2 = exp(-0.5*3.5*3.5*S2);
                col += w2 * tex2D(_MainTex, i.globalTexcoord + float2(3.5,0) * scale);
                col += w2 * tex2D(_MainTex, i.globalTexcoord - float2(3.5,0) * scale);
                weight +=2.0*w2;
                
                float w3 = exp(-0.5*5.5*5.5*S2);
                col += w3 * tex2D(_MainTex, i.globalTexcoord + float2(5.5,0) * scale);
                col += w3 * tex2D(_MainTex, i.globalTexcoord - float2(5.5,0) * scale);
                weight +=2.0*w3;
                
                float w4 = exp(-0.5*7.5*7.5*S2);
                col += w4 * tex2D(_MainTex, i.globalTexcoord + float2(7.5,0) * scale);
                col += w4 * tex2D(_MainTex, i.globalTexcoord - float2(7.5,0) * scale);
                weight +=2.0*w4;
                
                float w5 = exp(-0.5*9.5*9.5*S2);
                col += w5 * tex2D(_MainTex, i.globalTexcoord + float2(9.5,0) * scale);
                col += w5 * tex2D(_MainTex, i.globalTexcoord - float2(9.5,0) * scale);
                weight +=2.0*w5;
